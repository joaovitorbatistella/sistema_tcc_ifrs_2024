            ...
            <div class="sm:flex justify-between items-center">
                ...
                <div>
                    <button type="button" class="bg-c-green text-white text-lg font-bold py-1 px-2 rounded">
                        <a href="{{route('alunos-controller.create')}}">Adicionar Aluno</a>
                    </button>
                </div>
            </div>
        </x-slot>
                            ...
                            <tr>
                                <td class="p-2 border">
                                    <div class="text-center text-sm">
                                        <p class="font-semibold text-gray-700">{{$aluno->name}}</p>
                                    </div>
                                </td>
                                <td class="p-2 border">
                                    <div class="text-center text-sm">
                                        <p class="font-semibold text-gray-700">{{$aluno->email}}</p>
                                    </div>
                                </td>
                                <td class="p-2 border">
                                    <div class="text-center text-sm">
                                        <p class="font-semibold text-gray-700">{{$aluno->rg}}</p>
                                    </div>
                                </td>
                                <td class="p-2 border">
                                    <div class="text-center text-sm">
                                        <p class="font-semibold text-gray-700">{{$aluno->cpf}}</p>
                                    </div>
                                </td>
                                <td class="p-2 border">
                                    <form action="{{route('alunos-controller.destroy',['aluno' => $aluno->id])}}" method="POST">
                                        @csrf
                                        <input type="hidden" name="_method" value="DELETE">
                                        <div class="text-center text-sm">
                                            <a style="color: black;" href="{{route('alunos-controller.show',['aluno' => $aluno->id])}}" class="mr-2">
                                                <i class="material-icons-outlined text-base">visibility</i>
                                            </a>
                                            <a style="color:orange" href="{{route('alunos-controller.edit',['aluno' => $aluno->id])}}" class="mx-2">
